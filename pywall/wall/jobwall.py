...
class JobWall(Wall):
	...
	def __init__(self, canvas, views):
		Wall.__init__(self, canvas)
		self.views = views
		
	def updateInfo(self):
		self.jobs = get_view_status(self.views[self.itView % len(self.views)])
	...
	def paintJob(self, job, rect):
		...
		if(job['last_build'] == None or job['status'] == 'ABORTED'):
			color = '#3c3c3c'
		...
		if(status == 'BUILDING'):
			...
			self.canvas.create_text(posX, posY - (self.jobHeight / 2) + 30, text=completeRateText, font=buildNumberFont, fill='white', justify='center')
		...
		if(buildNumber is not None):
			...
			self.canvas.create_text(xCorner, yCorner, text=buildNumber, font=buildNumberFont, fill=buildNumberColor, anchor='w')	
			
		self.canvas.create_text(posX+3,posY+3, text=name, font=nameFont, fill='black', justify='center', width=self.jobWidth)
		self.canvas.create_text(posX, posY, text=name, font=nameFont, fill='white', justify='center',width=self.jobWidth)
